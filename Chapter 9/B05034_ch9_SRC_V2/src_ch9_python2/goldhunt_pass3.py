...
This module illustrates the third optimization pass for the Gold Hunt
program.

Here, we use goldhunt_pass2.py as the base. and do further improvements..
The function generate_random_points is modified to use locals
(and avoid globals)
...
def generate_random_points(ref_radius, total_points):
    ...
    .. note:: This function runs slightly faster compared to the one in
      earlier optimization pass (goldhunt_pass2). However the optimization
      for this function is not over yet. For a later optimization pass
      we will use NumPy arrays and other functions to speed this up!
    .. note:: Make sure such changes do not affect readability! See a note
      about this in the relevant chapter.
